...
def enrich_mail_account_ref(
    ...
    *,
    ...
    if operating_organization is not None:
        metadata["operating_organization_id"] = getattr(
    ...
    if email_identity is not None:
        metadata["email_identity_id"] = getattr(
            email_identity,
            "id",
            email_identity,
        )
...
def resolve_mail_account_ref(mailbox_account: MailboxAccount | int) -> MailAccountRef:
    if isinstance(mailbox_account, int):
        mailbox_account = MailboxAccount.objects.select_related(
            "operating_organization"
        ).get(pk=mailbox_account)
